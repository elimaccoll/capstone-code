...
export const addPlotPoint = (chart_name, data_point, series_ind = 0) => {
    ...
    // bufferData(chart_name, plot_point, series_ind);
    storeDataBuffer(chart_name, plot_point, series_ind);
}


const verifyPlotThresholds = (minThreshold, maxThreshold, minBound, maxBound) => {
    // Not a number (empty)
    if (isNaN(minThreshold) || isNaN(maxThreshold)) { return false; }
    if ((minThreshold < minBound) || (minThreshold >= maxThreshold)
        || (maxThreshold > maxBound) || (maxThreshold <= minThreshold)) {
        return false;
    }
    return true;
}
...
const handleThresholdUpdate = (chart, is_min) => {
    ...
    const threshold_value = parseFloat($(`#${chart.name}-${bound}-threshold`).val());
    ...
    if (is_min && verifyPlotThresholds(threshold_value, curr_maxThreshold, chart_config.minBound, chart_config.maxBound)) {
        chart_config.minThreshold = threshold_value;
        update = true;
    }
    // Condition for updating max threshold
    else if (!is_min && verifyPlotThresholds(curr_minThreshold, threshold_value, chart_config.minBound, chart_config.maxBound)) {
        chart_config.maxThreshold = threshold_value;
        update = true;
    }
    // Display the current threshold values in their respective input fields
    displayThresholds(chart.name, chart_config.minThreshold, chart_config.maxThreshold);
    ...
    storeThresholds(chart.name, chart_config.minThreshold, chart_config.maxThreshold);
    // Update drawn threshold zone for new thresholds on the plot
    drawUpdatedPlotThresholds(chart.name, chart_config.minThreshold, chart_config.maxThreshold);
    // Send updated thresholds to arduino
    sendPlotThresholds(chart_config.symbol, chart_config.minThreshold, chart_config.maxThreshold);
}
